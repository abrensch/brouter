...
public class ReuseClassifierTest {
  ...
  private static OsmTrack track(int[][] coords) {
    ...
    for (int i = 1; i < t.nodes.size(); i++) {
      MessageData m = RoundTripFixture.messageData("highway=residential surface=asphalt", 1.0f);
  ...
  public void gateRejectsProfileHostileScenicSpur() {
    ...
    OsmTrack t = outAndBack(6000);
    MessageData m = RoundTripFixture.messageData("highway=path", 5.0f);
    for (int i = 1; i < t.nodes.size(); i++) {
      MessageData copy = RoundTripFixture.messageData(m.getWayKeyValues(), m.getCostfactor());
