...
public class MapterhornSegmentTest {
  ...
  public void routedTrackCarriesTheMapterhornElevation() throws Exception {
    ...
    // 3. the real map-creation pipeline, exactly as MapcreatorTest runs it
    File tmpDir = tmp.newFolder("work");
    File nodes = mkdir(tmpDir, "nodetiles");
    File ways = mkdir(tmpDir, "waytiles");
    File nodes55 = mkdir(tmpDir, "nodes55");
    File ways55 = mkdir(tmpDir, "waytiles55");
    ...
    File segments = mkdir(tmpDir, "segments");
    new WayLinker().process(unodes55, ways55, bordernodes,
      new File(tmpDir, "restrictions.dat"), lookupFile,
    ...
    rc.localFunction = new File(profileDir, "trekking.brf").getAbsolutePath();
    List<OsmNodeNamed> waypoints = new RoutingParamCollector()
      .getWayPointList("8.6832,50.0074|8.7250,49.9916");
