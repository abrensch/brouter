...
public class OsmParser {
  ...
  public static void doParse(File lookupFile, File tmpDir, File profileAll, File profileReport, File profileCheck, File mapFile, String dbTagFilename) throws Exception {

    // first step: parse and cut to 45*30 degree tiles
    DenseLongMap nodeFilter = new OsmParser().process(lookupFile, tmpDir, profileAll, mapFile, dbTagFilename);
  ...
  public DenseLongMap process(File lookupFile, File tmpDir, File profileFile, File mapFile, String dbTagFilename ) throws Exception {
    ...
    new OsmPbfDecoder().readMap(mapFile, this);
...
//    _expctxNodeStat.dumpStatistics();
    ...
    try {
      ...
      for (int fi = 0; fi < fromWid.size(); fi++) {
        for (int ti = 0; ti < toWid.size(); ti++) {
          nextRestriction(r, fromWid.get(fi), toWid.get(ti), viaNid.get(0));
        }
      }
    } catch (Exception e) {
  ...
  public void addNode(NodeData n) throws IOException {
    nodesParsed++;
    checkStats();
  ...
  public void addWay(WayData w) throws IOException {
    waysParsed++;
    checkStats();
